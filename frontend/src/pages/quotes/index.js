...
const ListQuotes = (props) => {
	...
	useEffect(() => {
		...
		const initQuotes = async () => {
			const { data } = await listQuotes(search, filter === 'null' ? null : filter, limit, offset, year);
			setQuotes(data.data);
			getPagination(data.length, limit, setOffset, setPagination);
		}

		initQuotes();
	}, [isLoggedIn, listQuotes, filter, limit, offset, search, year]);
						...
						quotes !== null ?
							quotes.map((item, index) => {
								return (
									<tr key={index}>
										<td>
											<a href={`/quotes/${item._id}`} className="btn btn-secondary"><FontAwesomeIcon icon={faEdit} /></a>
